...
def __ReportGGGet(secID = "", ticker = "", writeDate = "", orgName = "", BeginWriteDate = "", EndwriteDate = "", title = "", author = "", field = "", pandas = "1"):
    ...
    if hasattr(orgName,'__iter__') and not isinstance(orgName, str):
        if len(orgName) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in orgName]))
    else:
        requestString.append(str(orgName) if not isinstance(orgName, unicode) else orgName)
    try:
    ...
    if hasattr(author,'__iter__') and not isinstance(author, str):
        if len(author) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in author]))
    else:
        requestString.append(str(author) if not isinstance(author, unicode) else author)
...
def __ReportadjustGGGet(ticker = "", RRID = "", currentCreateDate = "", beginDate = "", endDate = "", field = "", pandas = "1"):
    ...
    if hasattr(RRID,'__iter__') and not isinstance(RRID, str):
        if len(RRID) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in RRID]))
    else:
        requestString.append(str(RRID) if not isinstance(RRID, unicode) else RRID)
    try:
...
def __SWconForeGGGet(ticker = "", industry = "", beginDate = "", endDate = "", tmStamp = "", field = "", pandas = "1"):
    ...
    if hasattr(industry,'__iter__') and not isinstance(industry, str):
        if len(industry) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in industry]))
    else:
        requestString.append(str(industry) if not isinstance(industry, unicode) else industry)
    try:
        beginDate = beginDate.strftime('%Y%m%d')
    except:
        beginDate = beginDate.replace('-', '')
