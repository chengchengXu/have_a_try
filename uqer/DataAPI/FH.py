...
def __FXTickRTSnapshotGet(currencyPair, field = "", pandas = "1"):
    ...
    if hasattr(currencyPair,'__iter__') and not isinstance(currencyPair, str):
        if len(currencyPair) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in currencyPair]))
    else:
        requestString.append(str(currencyPair) if not isinstance(currencyPair, unicode) else currencyPair)
