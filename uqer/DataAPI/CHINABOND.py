...
def __YieldCurveGet(CurveName, Tenor, CurveType = "", BeginDate = "", EndDate = "", field = "", pandas = "1"):
    ...
    if hasattr(CurveName,'__iter__') and not isinstance(CurveName, str):
        if len(CurveName) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in CurveName]))
    else:
        requestString.append(str(CurveName) if not isinstance(CurveName, unicode) else CurveName)
    ...
    if hasattr(Tenor,'__iter__') and not isinstance(Tenor, str):
        if len(Tenor) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in Tenor]))
    else:
        requestString.append(str(Tenor) if not isinstance(Tenor, unicode) else Tenor)
...
def __CBBenchmarkIndicesGet(benchmarkName = "", valuationType = "", beginDate = "", endDate = "", field = "", pandas = "1"):
    ...
    if hasattr(benchmarkName,'__iter__') and not isinstance(benchmarkName, str):
        if len(benchmarkName) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in benchmarkName]))
    else:
        requestString.append(str(benchmarkName) if not isinstance(benchmarkName, unicode) else benchmarkName)
    ...
    if hasattr(valuationType,'__iter__') and not isinstance(valuationType, str):
        if len(valuationType) > 100 and split_param is None:
            ...
            requestString.append(','.join([str(item) if not isinstance(item, unicode) else item for item in valuationType]))
    else:
        requestString.append(str(valuationType) if not isinstance(valuationType, unicode) else valuationType)
    try:
        beginDate = beginDate.strftime('%Y%m%d')
    except:
        beginDate = beginDate.replace('-', '')
