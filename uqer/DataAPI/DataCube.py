...
from . data_service.utils.datetime_utils import get_trading_days, get_trading_days_by_period_type
...
from . data_service.const import (
    EQUITY_DAILY_FIELDS,
...
def uniform_date(input_date):
    if isinstance(input_date, basestring):
        ...
        try:
            ...
            print_message(u'输入的日期：%s 无法解析，请重新输入' % input_date)
        return format_date
    if isinstance(input_date, datetime.datetime) or isinstance(input_date, datetime.date):
        return input_date
    else:
        print_message(u'输入的日期格式不支持，请使用 datetime或string类型.')
        return


def get_data_cube(symbol, field, start, end=None, period_type=None, freq='1d', style='sat', adj=None, **kwargs):
    '''
        ...
        :param period_type: [str] 默认为空值，取值范围（'week_first', 'week_last', 'month_first', 'month_last'）
    ...
    if not (is_pro_user() or is_enterprise_user()):
        print_message(u"该API仅限优矿专业版、企业版使用，请联系优矿客服 service.uqer@datayes.com，谢谢！")
        return

    MKT_DAILY_FIELDS = set(EQUITY_DAILY_FIELDS) | set(STOCK_DAILY_FQ_FIELDS) | set(FUTURES_DAILY_FIELDS)

    MKT_INTRADAY_FIELDS = set(EQUITY_MINUTE_FIELDS) | set(FUTURES_MINUTE_FIELDS) - set(['tradeTime'])

    if not is_enterprise_user():
        MKT_DAILY_FIELDS = MKT_DAILY_FIELDS | set(ALIAS_DAILY_PRICE)
        MKT_INTRADAY_FIELDS = MKT_INTRADAY_FIELDS | set(ALIAS_MINUTE_PRICE)
    ...
    if period_type and period_type not in PERIOD_TYPE_LIST:
        print_message(u'传入的period_type参数值错误，仅支持:%s。' % PERIOD_TYPE_LIST)
        return
    ...
    if isinstance(freq, basestring):
        if freq not in ['1d', '1m', 'd', 'm', '5m', '15m', '30m', '60m']:
            print_message(u"目前只支持'1d','1m','5m', '15m', '30m', '60m'六个频率类型，暂不支持:%s。" % freq)
            return
    else:
        print_message(u"参数freq的值支持str类型，请校验后重新输入。")

    # validate adj
    if adj is not None and adj != 'pre':
        print_message(u"目前adj参数只支持'pre'，表示行情前复权，无法识别传入的adj参数值，默认返回未复权行情数据。")
    ...
    if freq in ['1m', '5m', '15m', '30m', '60m', 'm']:
        ...
        elif futures and start_date < datetime.datetime(2010, 1, 1):
            print_message(u"使用分钟线：期货支持的最早start为2010-01-01，自动修改为2010-01-01。")
    ...
    if set(field) - set(mkt_fields):
        factors = translate_factors(list(set(field) - set(mkt_fields)), start_date.strftime('%Y%m%d'), **kwargs)
    else:
        factors = []
    fs_fields = list(set(field) & set(FS_FIELD))
    rest = set(field) - set(mkt_fields) - set(factors) - set(fs_fields)
    if rest:
        print_message(u"传入的参数fields中有 %s无法识别，请校验后重试。" % list(rest))

    if period_type and fs_fields:
        print_message(u'以下字段(%s)不支持和参数period_type一起使用。' % fs_fields)
        fs_fields = []
    ...
    if end_date >= uniform_date(TODAY) and get_trading_days(TODAY_STR, TODAY_STR):
        ...
        if min([lastest_stock_tdate.strftime('%Y%m%d'), lastest_future_tdate.strftime('%Y%m%d')]) < TODAY_STR:
            print_message(u"今日行情还没有入库，默认获取截止到上一个交易日的数据。")
            end_date = uniform_date(YESTERDAY)

    if end_date < start_date:
        print_message(u"传入的start晚于end时间，请修正后重试。")
        return
    trading_days = [datetime.datetime.strptime(x, '%Y%m%d')
                    for x in get_td_from_cache(start_date.strftime('%Y%m%d'),
                                               end_date.strftime('%Y%m%d'))]
    if period_type:
        trading_days = get_trading_days_by_period_type(period_type, start_date, end_date)
        if not trading_days:
            print_message(u'根据start:%s ，end:%s 和period_type:%s 的输入值，得到的交易日列表为空' %
    ...
    if len(trading_days) == 0:
        print_message(u"传入的日期区间 start:%s, end:%s 不包含交易日，无数据返回" % (start, end))
        return
    ...
    for univ in stocks + funds:
        if univ in stock_fund_tdates_dict:
            for one_tuple in stock_fund_tdates_dict[univ]:
                if str(one_tuple[0]) <= end_date_str and (one_tuple[1] is None or str(one_tuple[1]) > start_date_str):
...
#             filtered_stock_fund.append(univ)
    if len(set(stocks) - set(filtered_stock_fund)) > 0:
        print_message(u"传入的股票列表中：%s，由于未上市或已退市，在给定的时间段内没有交易，数据不会出现在结果中。" %
               list(set(stocks) - set(filtered_stock_fund)))
    if len(set(funds) - set(filtered_stock_fund)) > 0:
        print_message(u"传入的基金列表中：%s，由于未上市或已退市，在给定的时间段内没有交易，数据不会出现在结果中。" %
    ...
    if futures:
        ...
        if len(set(futures) - set(filtered_futures)) > 0:
            print_message(u"传入的期货列表中：%s，由于未上市或已退市，在给定的时间段内没有交易，数据不会出现在结果中。" %
    ...
    if invalid_symbol:
        print_message(u"无法识别传入的symbol参数中的%s，请检查后重试。" % invalid_symbol)
    ...
    if freq in ['1m', '5m', '15m', '30m', '60m', 'm']:
        if 'IFZ0' in symbol:
            print_message(u"IFZ0暂不支持分钟线数据。")
            symbol.remove('IFZ0')

        mkt_daily_fields = list(set(mkt_daily_fields) - set(mkt_minute_fields))

        if mkt_daily_fields:
            print_message(u"传入字段：%s为日线字段，暂时没有分钟相关的数据。" % mkt_daily_fields)
        market_service = MarketService.create_with_service(symbol,
                                                           [],
                                                           mkt_minute_fields,
                                                           factors,
